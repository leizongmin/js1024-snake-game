var w = 20, h = 20, pw = a.width / w, ph = a.height / h;
...
c.mdo = function (f, a) { return c[f].apply(c, a), c; };
c.mset = function (f, v) { return c[f] = v, c; };
function drawPoint (p, a, b, color) { c.mset('strokeStyle', '#fff').mdo('strokeRect', [p.x * pw, p.y * ph, pw, ph]).mset('fillStyle', color || '#aaa').mdo('fillRect', [p.x * pw, p.y * ph, pw, ph]); }
function rndp () {
  var p = {x: parseInt(Math.random() * w, 10), y: parseInt(Math.random() * h, 10)};
  return pb.some(function (p2) { return p.x === p2.x && p.y === p2.y; }) ? rndp() : p;
}
function gameOver () { c.mset('fillStyle', '#e96900').mset('font', 'bold 24px 宋体').mdo('fillText', ['Game Over', (a.width - 24 * 4) / 2, (a.height - 24) / 2]); }
function nxp (p) {
  var d = D[p.d];
...
function loop () {
  ...
  for (var i = 0; i < pb.length; i++) if (hp.x === pb[i].x && hp.y === pb[i].y) return gameOver();
  pb.unshift(hp); // 没有吃到食物，蛇头前进时，蛇尾要收缩
  (pf.x === hp.x && pf.y === hp.y) ? pf = rndp() : pb.pop();
  c.mset('fillStyle', '#f1fedd').mdo('fillRect', [0, 0, a.width, a.height]);
  drawPoint(pf, 0, 0, '#e96900'); // 食物
  drawPoint(pb[0], 0, 0, '#2d64b3'); // 蛇头
  pb.slice(1).forEach(drawPoint); // 舍身
  tid = setTimeout(loop, 200 - pb.length * 1.5); // 自动加快速度
}
var tid = setTimeout(loop, 200);